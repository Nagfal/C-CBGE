...
# version ='1.0'
# ---------------------------------------------------------------------------
""" The running environment of C-CBGE"""  


import numpy
import sympy
import agent_CCBGE as agent
...
class bandit(object):
    ...
    # input variables:
    # eta : the difference parameter (eta = 0 for Scenario 1 and eta > 0 for Scenario 2 )(Fig. 2)
    # sd : if sd> 0 then it gives the standard deviation of the expected rewards in Scenario 2 (Fig. 3), to run Fig.2 set sd =-1
    # sigma : the variance of the reward (sigma >= 0)
    # exploration_constant : the exploration constant (exploration_constant = 1.0)
    # social_network_mode : the type of the social network ('full','cycle','stochastic')
    # stochastic_network_prob : the probability of two agents being the neighbors (only in stochastic)
    # return:
    # null
    #-------------------------------------------------------------------------------- 
    def reset(self,eta = 0.0, sd = -1, sigma = 1.0, exploration_constant = 1.0, social_network_mode = 'full',stochastic_network_prob = 0.2):
        assert social_network_mode in ['full','cycle','stochastic']
        self.eta = eta
        self.sigma = sigma
        ...
        if eta > 0.0:
            for i in range(1,len(self.origin_expected_reward_list)):
                for j in range(0, len(self.origin_expected_reward_list[i])):
                    self.origin_expected_reward_list[i][j] = numpy.random.normal(self.origin_expected_reward_list[0][j],self.sd)
            for j in range(0, len(self.origin_expected_reward_list[0])):
                self.origin_expected_reward_list[0][j] = numpy.random.normal(self.origin_expected_reward_list[0][j],self.sd)
    ...
    def social_network_init(self, social_network_mode):
        for i in range(0,self.agent_num):
            self.agent_list.append(agent.agent(i, self.arm_num,self.eta,exploration_constant=self.exploration_constant))
        
        if social_network_mode == 'full':
            for a in range(len(self.agent_list)):
                self.agent_list[a].neighbors = self.agent_list.copy()
                self.agent_list[a].neighbors.pop(a)
        elif social_network_mode == 'cycle':
    ...
    def round(self):
        ...
        for a in range(0,len(self.agent_list)):
            ...
            # if arm == 0:
            #     # reward = numpy.random.normal(self.origin_expected_reward_list[a][arm],self.sigma)
            #     reward = numpy.random.choice([0,1],p=[(1-self.origin_expected_reward_list[a][arm]) , self.origin_expected_reward_list[a][arm]])
            # else:
            #     reward = self.origin_expected_reward_list[a][arm]
